...
TEMPLATE_LAYOUT () {
	...
	if [ -f "${1}.TITLE" ]; then
		TEMPLATE "${LAYOUTS}/_Layout.html" 'TITLE' "${1}.TITLE" "${TMP}"
		TEMPLATE "${TMP}" 'PLACEHOLDER' "${1}" "${2}"
	else
		echo "No TITLE metadata found for file ${1}; skipping"
	fi
