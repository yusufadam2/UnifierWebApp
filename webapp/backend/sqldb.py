...
def bootstrap():
    ...
    CREATE_INTEREST_QUERY = 'INSERT INTO Interests (name) VALUES (?);'
    interests = ['sport', 'science', 'food', 'music', 'reading', 'hiking']
    for i in interests:
        do_sql(cur, CREATE_INTEREST_QUERY, (i,))

    JOIN_USER_INTEREST_QUERY = 'INSERT INTO UsersInterestsJoin (userId, interestId) VALUES (?,?);'
    johns_interests = [1, 3, 4]
    for i in johns_interests:
        do_sql(cur, JOIN_USER_INTEREST_QUERY, (1, i))

    janes_interests = [2, 5, 6]
    for i in janes_interests:
        do_sql(cur, JOIN_USER_INTEREST_QUERY, (2, i))

    foobars_interests = [1, 2, 3, 4, 5, 6]
    for i in foobars_interests:
        do_sql(cur, JOIN_USER_INTEREST_QUERY, (3, i))
